...
import { relativeReportPath, toPosixPath, wikiPath, wikiwikiPath } from "./paths";
...
export function applyCompileDraft(root: string, draftId: string): CompileApplyResult {
  ...
  for (const page of manifest.pages) {
    ...
    renderedFiles.push(toPosixPath(page.output_path));
  }
...
function buildPage(
  ...
  const sourceFiles = unique(selectedRecords.flatMap(relatedFiles).map(toPosixPath));
