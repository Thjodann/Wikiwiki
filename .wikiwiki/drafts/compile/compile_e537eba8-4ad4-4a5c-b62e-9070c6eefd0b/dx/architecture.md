...
Wikiwiki has four main layers.

The storage layer keeps records as JSONL files under `.wikiwiki/records/`. Append-only revisions preserve audit history, while active reads select the latest non-deleted revision.

The validation layer checks record shape, revision order, tombstones, links, deterministic rendering, and compiled human pages when they exist.

The rendering layer turns active records into deterministic Markdown pages such as concepts, decisions, notes, symbols, and links. These pages are generated views over the structured store.

The compile layer creates role-oriented draft packets for UX and DX human wikis. It routes records by tags and shared authority, writes a manifest, produces draft Markdown for an IDE agent, and validates provenance before publishing to `wiki/human/`.

This architecture keeps the model simple: records are truth, generated pages are views, and human prose is accepted only through a visible draft/apply path.
